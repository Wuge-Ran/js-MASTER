...
const getSingerList = (params = { classid: '' }) => {
  return request(`/singer/list/${params.classid}?json=true`)
}

// 根据歌手id，获取歌手歌曲

const getSingerInfo = (params = { singerid: '' }) => {
  return request(`/singer/info/${params.singerid}?json=true`)
}

//根据排行榜id,获取排行榜歌曲

const getRankInfo = (params = { rankid: '' }) => {
  return request(`/singer/info/${params.rankid}?json=true`)
}
